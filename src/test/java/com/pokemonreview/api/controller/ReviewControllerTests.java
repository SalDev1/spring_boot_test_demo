...
// Helps you to remove the default security filters to conduct tests securely.
...
public class ReviewControllerTests {
    ...
    public void ReviewController_GetReviewsByPokemonId_ReturnReviewDto() throws Exception {
        int pokemonId = 1;
        // A pokemon can have multiple reviews.
    ...
    public void PokemonController_UpdateReview_ReturnReviewDto() throws Exception {
        ...
        response.andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.name", CoreMatchers.is(reviewDto.getTitle())))
                .andExpect(MockMvcResultMatchers.jsonPath("$.type", CoreMatchers.is(reviewDto.getContent())))
    ...
    @Test
    public void ReviewController_GetReviewsId_ReturnReviewDto() throws Exception {
        int pokemonId = 1;
        int reviewId = 1;
        when(reviewService.getReviewById(reviewId,pokemonId)).thenReturn(reviewDto);

        ResultActions response = mockMvc.perform(post("/api/pokemon/1/reviews/1")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(pokemonDto)));
    ...
    public void ReviewController_DeleteReview_ReturnOk() throws Exception {
        ...
        ResultActions response = mockMvc
                .perform(get("/api/pokemon/1/reviews/1")
                        .contentType(MediaType.APPLICATION_JSON));
